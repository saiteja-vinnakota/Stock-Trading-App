...
function StockDetails() {
  ...
  const fetchStock = async () => {
    try {
      const stockRes = await axiosInstance.get(`/stocks/details/${id}`);
      setStock(stockRes.data.stock);
      setOwnedQuantity(stockRes.data.ownedQuantity);
  ...
  const sellStock = async () => {
    try {
      if (quantity > ownedQuantity) {
        toast.error(`You only own ${ownedQuantity} shares.`);
        return;
      }
  ...
  const toggleWatchlist = async () => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Unable to update watchlist.",
      );
    }
  };
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="bg-white rounded-3xl shadow-sm border p-8">
            <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-8">
              <div>
                ...
                <p className="text-gray-500 mt-3 text-lg">{stock.name}</p>
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-gray-500 mt-2">Live Market Price</p>
                <button
          ...
          <div className="grid lg:grid-cols-3 gap-6 mt-8">
            ...
            <div className="bg-white rounded-3xl shadow-sm border p-6 h-fit">
              ...
              <div className="bg-slate-100 rounded-xl p-4 mb-6">
                <p className="text-gray-500 text-sm">Owned Shares</p>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-8">
            <div className="bg-white rounded-2xl border shadow-sm p-6">
              ...
              <h3 className="text-2xl font-bold mt-3">{stock.sector}</h3>
            </div>
            <div className="bg-white rounded-2xl border shadow-sm p-6">
              <p className="text-gray-500">Exchange</p>
              <h3 className="text-2xl font-bold mt-3">{stock.stockExchange}</h3>
            </div>
            <div className="bg-white rounded-2xl border shadow-sm p-6">
